...
import (
	...
	bitmapallocator "sigs.k8s.io/cluster-api-provider-kubevirt/pkg/allocator/bitmap"
	ipallocator "sigs.k8s.io/cluster-api-provider-kubevirt/pkg/allocator/ip"
)
...
type SubnetConfig struct {
	...
	// RangeConstraints contains the original range constraints for range-aware allocation
...
type Allocator interface {
	...
	// AllocateNextIPsWithRangeConstraints allocates IPs respecting original range constraints
	AllocateNextIPsWithRangeConstraints(name string) ([]*net.IPNet, error)
...
type subnetInfo struct {
	...
	// rangeConstraints holds the original range constraints for range-aware allocation
	rangeConstraints []*RangeConstraint
}
...
func (allocator *allocator) AddOrUpdateSubnet(config SubnetConfig) error {
	...
	if subnetInfo, ok := allocator.cache[config.Name]; ok && !reflect.DeepEqual(subnetInfo.subnets, config.Subnets) {
		klog.Warningf("Replacing subnets %v with %v for %s", StringSlice(subnetInfo.subnets), StringSlice(config.Subnets), config.Name)
	}
	...
	for _, subnet := range config.Subnets {
		...
		if utilnet.IsIPv4CIDR(subnet) {
			subnetBoundaryIPs = append(subnetBoundaryIPs, subnet.IP, SubnetBroadcastIP(*subnet))
		}
	}
	...
	for _, excludeFromIPAM := range append(config.ReservedSubnets, config.ExcludeSubnets...) {
		...
		for i, subnet := range config.Subnets {
			if ContainsCIDR(subnet, excludeFromIPAM) {
	...
	allocator.cache[config.Name] = subnetInfo{
		...
		rangeConstraints: config.RangeConstraints,
	}
	return nil
}
...
func (allocator *allocator) AllocateIPPerSubnet(name string, ips []*net.IPNet) error {
	...
	subnetInfo, ok := allocator.cache[name]
	if !ok {
		return fmt.Errorf("failed to allocate IPs %v for %s: %w", StringSlice(ips), name, ErrSubnetNotFound)
	} else if len(subnetInfo.ipams) == 0 && len(subnetInfo.staticIPAMs) == 0 {
		return fmt.Errorf("failed to allocate IPs %v for subnet %s: has no IPAM", StringSlice(ips), name)
	}
...
// AllocateNextIPsWithRangeConstraints allocates IPs respecting original range constraints
func (allocator *allocator) AllocateNextIPsWithRangeConstraints(name string) ([]*net.IPNet, error) {
	allocator.RLock()
	defer allocator.RUnlock()
	var ipnets []*net.IPNet
	var ip net.IP
	...
	// If no range constraints are available, fall back to regular allocation
	if len(subnetInfo.rangeConstraints) == 0 {
		return allocator.AllocateNextIPs(name)
	}
	...
	defer func() {
		if err != nil {
			...
			for relIdx, relIPNet := range ipnets {
				subnetInfo.ipams[relIdx].Release(relIPNet.IP)
				if relIPNet.IP != nil {
					klog.Warningf("Range-constrained IP %s was released for %s", relIPNet.IP, name)
				}
			}
		}
	}()
	...
	for idx, ipam := range subnetInfo.ipams {
		// Find the corresponding range constraint for this subnet
		var constraint *RangeConstraint
		if idx < len(subnetInfo.rangeConstraints) {
			constraint = subnetInfo.rangeConstraints[idx]
		}

		// Try to allocate an IP that respects the range constraint
		ip, err = allocator.allocateNextIPWithConstraint(ipam, constraint)
		if err != nil {
			if errors.Is(err, ipallocator.ErrFull) {
				err = fmt.Errorf("failed to allocate new IPs for %s: %w", name, ipallocator.ErrFull)
			}
...
// allocateNextIPWithConstraint allocates the next IP that satisfies the range constraint
func (allocator *allocator) allocateNextIPWithConstraint(ipam ipallocator.ContinuousAllocator, constraint *RangeConstraint) (net.IP, error) {
	// If no constraint, use regular allocation
	if constraint == nil {
		return ipam.AllocateNext()
	}

	// For CIDR constraints, regular allocation should work fine
	if constraint.Type == RangeTypeCIDR {
		return ipam.AllocateNext()
	}

	// For hyphenated and single IP constraints, we need to check if the allocated IP is within range
	...
	for attempts := 0; attempts < maxAttempts; attempts++ {
		...
		// Check if the allocated IP is within the original range constraint
		if constraint.Contains(ip) {
			return ip, nil
		}

		// If not in range, release it and try again
		ipam.Release(ip)
	}

	return nil, fmt.Errorf("failed to allocate IP within range constraint %s after %d attempts", constraint, maxAttempts)
}
